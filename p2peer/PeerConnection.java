...
public class PeerConnection extends Thread {
	...
	public void handleMsg(Message msg) {
		if (msg.type == Message.MessageType.choke) {
			...
			// Suppose peer A receives a unchoke message from peer B. Peer A selects a piece
			// randomly among the pieces that peer B has and peer A does not have and also
			...
			for (int i : file_wrapper.getDefectSubFiles()) {
			...
			if (preferredNeighbors.contains(otherPeerID) || optNeighbor == otherPeerID) {
				int pieceIndex = msg.payload.getInt();
				file_wrapper.send_piece_message(pieceIndex, out);
			}
			...
			logging.download(myPeerID, otherPeerID, pieceIndex,
					file_wrapper.pieceNum - file_wrapper.getDefectSubFiles().size());
	 ...
	 *
	 * @param msg
	 * @param isLastMsg Used for choking message log, if it is the last message,
	 *                  then log it.
	 * @param peerList  Used for choking message log, current peer has sent choke
	 *                  message to the peers in the list.
	 */
